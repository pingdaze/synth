...
import {deployCore1155, deployMock1155, deployAuctionValidator} from "./shared/deploys"
...
describe("AuctionValidator", () => {
  ...
  describe("isValid", () => {
    ...
    beforeEach(async () => {
        ...
        validator.createDrop(coreId, 10000, timestampBefore, timestampBefore+10000000, blockNumBefore+100);
    });
    ...
    it("can't place a bid if there isn't enough deposit", async () => {
      await expect(validator.addBid(1, owner.address)).to.be.revertedWith("Sorry, that's not enough for the deposit");
    });
    it("can't declare a winner if the end time hasn't passed", async () => {
      await ethers.provider.send('evm_increaseTime', [1000]);
      await ethers.provider.send('evm_mine', []); 
      await expect(validator.setWinner(1, owner.address)).to.be.revertedWith("SEQ_DROP_TIME_EARLY");
    ...
    it("can't claim the winning bid if you arn't a winner", async () => {
      ...
      await ethers.provider.send('evm_increaseTime', [1000]);
      await ethers.provider.send('evm_mine', []); 
      await expect(validator.connect(user1).claimAuction(1)).to.be.revertedWith("Sorry your bid is not a winner bb, get ur weight up");
