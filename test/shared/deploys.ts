...
export const deployCore1155 = async (
    ...
    if(authority != zeroAddress) {
      auth = await ethers.getContractAt("RolesAuthority", authority) as RolesAuthority;
    } else {
      const RolesAuthority = await ethers.getContractFactory("RolesAuthority");
      auth = await RolesAuthority.deploy(owner.address, zeroAddress) as RolesAuthority;
    }      
...
export const deployAuctionValidator = async (
      core: Core1155
  ) => {
...
export const deployBoosterValidator = async (
    ...
    return BoosterValidator.deploy(core.address, coordinator.address, link.address);
  };  
  

export const deployCharacterValidator = async (
      core: Core721, options: SelectableOptions, wearablesValidator: WearablesValidator, augmentsValidator : AugmentsValidator, character: Characters, requester : RandomnessRelayL2, charCount = 10
  ) => {
    const CharacterValidator = await ethers.getContractFactory("CharacterValidator");
    return CharacterValidator.deploy(core.address, options.address, wearablesValidator.address, augmentsValidator.address, character.address, charCount, requester.address );
  };
...
export const deployRegistry = async (authority: Authority | string = zeroAddress) => {
    const Registry = await ethers.getContractFactory("MetadataRegistry");
    return Registry.deploy(getAuthAddr(authority));
  }

  
export const deploySequenceValidator = async (
    core: Core1155
  ) => {
