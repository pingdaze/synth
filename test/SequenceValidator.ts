...
describe("SequenceValidator", () => {
  ...
  describe("createDrop", () => {
    ...
    it("fails if drop exists", async () => {
      const endBlock = await getEndBlock();
      expect( validator.createDrop(id, amount, startTime, endTime, endBlock)).to.be.revertedWith("SEQ_DROP_DUPLICATE_DROP");
    });
  });
  describe("isValid", () => {
    let validator: SequenceValidator;
    ...
    it("fails if more than one token is requested", async () => {
      expect(
        validator.validate(user1.address, [id], [2], "", [])).to.be.revertedWith(
        "SEQ_DROP_LIMIT_ONE"
      );
    });
    it("fails if the request would exceed that available quantity", async () => {
      expect(
        validator.validate(user1.address, [id], [6], "", [])).to.be.revertedWith(
