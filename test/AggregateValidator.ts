...
import {PaymentValidator, AggregateValidator, MultiRolesAuthority, CheckerValidator, Core1155, Basic1155} from "../typechain-types/";
...
import {deployPaymentValidatorV2, deployMultiAuth, deployCheckerValidatorV2, deployMock1155,  deployCore1155, deployAggregateValidator} from "./shared/deploys";
import { validators } from "../typechain-types/contracts";
...
describe.only("AggregateValidator", () => {
  ...
  describe.only("isValid", () => {
    ...
    beforeEach(async () => {
        ...
        validator = await deployAggregateValidator(core, coreId, supply, authority) as AggregateValidator;
        // Deploy the checkers that we're going to aggregate
        payments = await deployPaymentValidatorV2(validator, coreId, cost) as PaymentValidator;
        ...
        checker = await deployCheckerValidatorV2(validator, nift, coreId) as CheckerValidator;
        // Setup all of the authorities for the aggregator
        const AggregateInterface = new ethers.utils.Interface(AggregateValidatorABI.abi);
        const aggregateSig = AggregateInterface.getSighash(AggregateInterface.getFunction("modularMintCallback"));
    ...
    it("ensure the current validator is valid in core", async () => {
      const newValidator = await deployAggregateValidator(core, coreId, supply, authority) as AggregateValidator;
