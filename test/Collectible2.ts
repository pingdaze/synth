...
import {deployCore1155, deployBasicValidator, deployHolder, deployRegistry} from "./shared/deploys"
...
describe("Core", () => {
  ...
  describe("Validators", () => {
    ...
    it("can add a validator", async () => {
      ...
      expect(core.addValidator(validator.address, [id1])).to.emit(core, 'Validator')
    ...
    it("can remove a validator", async () => {
      expect(await core.isValidator(validator.address)).to.equal(true);
      expect(core.removeValidator(validator.address)).to.emit(
        validator, "Validator"
    ...
    describe("modular mint", () => {
      ...
      it("reverts if passed an inactive Validator Address", async () => {
        expect( core.modularMintInit(id, user1.address, [amount], "", zeroAddress, "")).to.be. revertedWith(
          "BAD_VALIDATOR"
        );
      });
      it("reverts if the validator doesn't succeed", async () => {
        ...
        expect(core.modularMintInit(badID, user1.address,[amount], "", badValidator.address, "")).to.be.revertedWith(
          "INVALID_MINT"
        );
      });
      it("mints tokens to a contract", async () => {
        ...
        await expect(core.modularMintInit(
          id3,
          holder.address,
          [amount],
          ...
          validator.address,
          ""
        )).to.emit(core, "TransferBatch").withArgs(
           validator.address,
           zeroAddress,
           holder.address,
           [id3],
      ...
      it("mints tokens to an EOA", async () => {
        ...
        const tx = await expect(core.modularMintInit(
          id3,
          user1.address,
          [amount],
          ethers.constants.HashZero,
  ...
  describe("batch minting", () => {
    ...
    it("can only be utilized by owners", async () => {
      const core = await deployCore1155();
      expect( core.connect(user2).mintBatch(user1.address, [id4], [amount], 0)).to.be.revertedWith(
