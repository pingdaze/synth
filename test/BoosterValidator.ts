...
import {BoosterValidator, Core1155, LinkMock, VRFCoordinatorMock} from "../typechain-types";
import {deployCore1155, deployLink, deployCoordinator, deployBoosterValidator} from "./shared/deploys"
...
describe("BoosterValidator", () => {
  ...
  describe("createDrop", () => {
    ...
    before(async () => {
      ...
      validator = await deployBoosterValidator(core, coordinator, link) as BoosterValidator;
      await validator.createDrop([id], [100], zeroAddress, 0, startTime, endTime, endBlock);
    });
    ...
    it("fails if drop exists", async () => {
      await expect(validator.createDrop([id], [100], zeroAddress, 0, startTime, endTime, endBlock)).to.revertedWith("BOOST_DROP_DUPLICATE_DROP");
    });
  });
  describe("isValid", () => {
    let core : Core1155;
    let link : LinkMock;
    let coordinator : VRFCoordinatorMock;
    let endBlock: number;  beforeEach(async () => {
    endBlock = await getEndBlock();
    core = await deployCore1155() as Core1155;
    link = await deployLink() as LinkMock;
    coordinator = await deployCoordinator(link) as VRFCoordinatorMock;
    ...
    link.mint(validator.address, web3.utils.toWei("100")).catch(error => console.error(error));
  ...
  it("fails if more than one type of token is requested", async () => {
    await validator.createDrop([id], [100], zeroAddress, 0, startTime, endTime, endBlock);
    (await expect(validator.validate(user1.address, 0, [1, 1], "", [])).to.be.revertedWith("BOOST_DROP_REQUEST_AMOUNTS_LENGTH"));
  });
  });
  ...
  it("mints tokens to a EOA with 100% odds", async () => {
    ...
    */
    await coordinator.callBackWithRandomness(receipt.logs[3].data, 12341234, validator.address);
