...
describe("CheckerValidator", () => {
  ...
  describe("isValid", () => {
    ...
    beforeEach(async () => {
        ...
        validator = await deployCheckerValidator(core, nift, coreId) as CheckerValidator;
    ...
    it("Can't redeem without holding mocked 1155", async () => {
        expect(validator.connect(user1).redeemAll([id1])).to.be.revertedWith("Sender must hold all tokens for migration");
    });
    ...
    it("can't redeem the same id twice", async () => {
      receipt = await validator.redeemAll([id1]);
      expect(validator.redeemAll([id1])).to.be.revertedWith("Token has already been redeemed");
    });
  });
});
