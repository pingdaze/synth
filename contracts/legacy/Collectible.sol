...
contract Collectible is ERC1155MixedFungible {
    ...
    modifier isNonFungibleType(uint256 _id) {
        ...
        require(
            (_id & TYPE_NF_BIT == TYPE_NF_BIT) && (_id & NF_INDEX_MASK == 0),
            "NOT_TYPE"
        );
        _;
    }
    ...
    function uri(uint256 _id) public view returns (string memory) {
        ...
        return
            string(abi.encodePacked(baseMetadataURI, uris[_id & SUBTYPE_MASK]));
    }
    ...
    function mint(
        ...
        address[] calldata _to
    ) external isNonFungibleType(_type) creatorOnly(_type) {
        ...
        string memory fullURI = string(
            abi.encodePacked(baseMetadataURI, _uriId)
        );
        // Index are 1-based.
     ...
     */
    function setURI(string calldata _uriId, uint256 _id)
        external
        creatorOnly(_id)
    {
        string memory fullURI = string(
